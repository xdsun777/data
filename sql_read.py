...
# 根据uid查询，去重
sql__all = """
SELECT id,主播昵称,时间, 用户昵称, 动作,uid, 抖音号, 性别, 地区, 简介, 勋章等级,粉丝, 关注, 精准, sec_uid,创建时间,省份 FROM "main"."zhibo"  GROUP BY "uid";
"""
["序号", "主播昵称", "时间", "用户昵称", "动作", "Uid", "抖音号", "性别", "地区", "简介", "等级",
 "粉丝", "关注", "精准", "Secid", "创建时间", "省份"]
# 指定日期查询
sql__date = """
SELECT "_rowid_",* FROM "main"."zhibo" WHERE "时间" REGEXP '2024-12-1[6789]' GROUP BY "Uid"
UNION
SELECT "_rowid_",* FROM "main"."zhibo" WHERE "时间" REGEXP '2024-12-2[01234]' GROUP BY "Uid" ORDER BY "主播昵称";
"""
...
def saveResult(data,save_path):
    ...
    for i in data:
        ...
        head = ["序号", "主播昵称", "时间", "用户昵称", "动作", "uid", "抖音号", "性别", "地区", "简介", "勋章等级",
                "粉丝", "关注", "精准", "Secid", "创建时间", "省份"]
