...
def dy_live(input_files="/*test_excel_dir*/",out_file=f'{time.strftime("%Y-%m-%d")}直播采集.xlsx'):
    ZHIBO = f'SELECT id,主播昵称,用户昵称,勋章等级,动作,抖音号,sec_uid,uid,简介,粉丝,关注,性别,地区,精准,时间,创建时间,省份 FROM "main"."zhibo" WHERE "时间" LIKE "%2025-01-07%" ESCAPE "\\" GROUP BY "uid" ORDER BY "省份";'
    '''sql:自由组合'''
...
if __name__ == '__main__':
    start = time.time()
    dy_live(input_files='C:\\Users\\ly\\Desktop\\work\\source\\ly直播采集\\20250107直播间采集结果.xlsx',out_file='20250107直播数据.xlsx')
